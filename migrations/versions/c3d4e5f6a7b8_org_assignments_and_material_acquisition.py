"""Org assignments (todos + materials) and material acquisition
...
def upgrade():
    ...
    with op.batch_alter_table(table_name('materials'), schema=None) as batch_op:
        batch_op.add_column(sa.Column('acquisition_labels', sa.JSON(), nullable=True))
        batch_op.add_column(sa.Column('sum_strategy', sa.Enum(
            'sum', 'max', name='sum_strategy', native_enum=False),
            nullable=False, server_default=sa.text("'sum'")))

    op.create_table(table_name('material_assignments'),
    sa.Column('material_id', sa.Integer(), nullable=False),
    sa.Column('org_id', sa.Integer(), nullable=False),
    sa.ForeignKeyConstraint(['material_id'], [_fk('materials.id')], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['org_id'], [_fk('orgs.id')], ),
    sa.PrimaryKeyConstraint('material_id', 'org_id')
    )
    with op.batch_alter_table(table_name('material_assignments'), schema=None) as batch_op:
        batch_op.create_index(batch_op.f(_ix('ix_material_assignments_org_id')), ['org_id'], unique=False)


def downgrade():
    with op.batch_alter_table(table_name('material_assignments'), schema=None) as batch_op:
        batch_op.drop_index(batch_op.f(_ix('ix_material_assignments_org_id')))
    op.drop_table(table_name('material_assignments'))

    with op.batch_alter_table(table_name('materials'), schema=None) as batch_op:
        batch_op.drop_column('sum_strategy')
        batch_op.drop_column('acquisition_labels')
